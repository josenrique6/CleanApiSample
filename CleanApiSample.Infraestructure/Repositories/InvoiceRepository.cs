...
namespace CleanApiSample.Infraestructure.Repositories
{
    public class InvoiceRepository : IInvoiceRepository
    {
        ...
        public Task<Invoice?> GetById(int id, CancellationToken cancellationToken)
        {
        ...
        public Task<List<Invoice>> ListAsyncByFilters(DateTime? fechaInicio, DateTime? fechaFin, string? ruc, CancellationToken cancellationToken)
        {
            ...
            {
                query = query.Where(w => w.Date.Date >= fechaInicio && w.Date.Date <= fechaFin);
            }

            if (!string.IsNullOrEmpty(ruc))
            {
                query = query.Where(w => w.CustomerRuc == ruc);
            }
            ...
            /*return _db.Invoices.AsNoTracking()
                .Include(i => i.Details)
                .Where(w=> ((fechaInicio == null && fechaFin == null) || (w.Date.Date >= fechaInicio && w.Date.Date <= fechaFin)) 
                || (ruc == null || w.CustomerRuc == ruc)) 
                .ToListAsync(cancellationToken);*/
        }
